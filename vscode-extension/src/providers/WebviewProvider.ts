...
import { ApiClient, Task, TeamMember } from '../services/ApiClient';

export class PrismWebviewProvider implements vscode.WebviewViewProvider {
    public static readonly viewType = 'prism.dashboard';
    ...
    private async handleStartTask(taskDescription: string, files: string[]) {
        try {
            ...
            if (!analysis.canProceed) {
                vscode.window.showWarningMessage(
                    'Potential conflicts detected! Review warnings before proceeding.',
                    'View Details',
                    'Proceed Anyway'
                ).then(selection => {
    ...
    private async createTask(taskDescription: string, files: string[]) {
    ...
    private async handleUpdateTaskStatus(taskId: string, status: string) {
        await this.apiClient.updateTask(taskId, { status: status as any });
        this.updateWebviewContent();
    }

    private showConflictDetails(warnings: any[]) {
        const panel = vscode.window.createWebviewPanel(
            'prismConflictDetails',
            'Conflict Analysis',
    ...
    public async showStartTaskPanel() {
    ...
    private getCurrentWorkspaceFiles(): string[] {
...
<body>
    ...
    private getConflictDetailsHtml(warnings: any[]): string {
        return `<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
